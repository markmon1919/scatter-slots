...
def compare_data(prev, current):
    ...
    if prev and 'jackpot_meter' in prev:
        ...
        print(f'\n\n\t----------------------------------')
        print(f"\t\t[ \033[1m{current['name'].upper()}\033[0m ] - {PROVIDERS.get(state.provider)}")
    ...
    for index, (period, value) in enumerate(current['history'].items()):
        ...
        if old_value is not None:
            ...
            if new_num is not None and old_num is not None:
                ...
                if index == 0 or index == 1 and new_num < old_num:
                    if index == 0:
                        new_num_10m = new_num
                        old_num_10m = old_num
                        delta_10m = delta
                        ...
                        if not new_delta_10m_1h < old_delta_10m_1h:
                        ...
                        result = {
                            ...
                            'prev_delta_10m_1h': old_delta_10m_1h,
                            'delta_shift': delta_shift,
                            'pull_score': score,
                            'pull_trend': trend or 'Neutral'
                        }

                        state.last_trend = f"{score} {result.get('pull_trend')}"

                        # play_alert(say="pull_trend_score")

                        if score >= 4: # ≥ 4 Strong Pull / Reversal Potential
                            bet_level = "high"
                        elif score >= 2 and score <= 3: # 2–3 Moderate Pull
                            bet_level = "mid" 
                        elif score >= 1 and score <= 0: # 0-1 Neutral
                            bet_level = "low" 
    ...
    if result is not None:
        ...
        if pull_score >= 8:
            ...
            trend_strength = "🔴 Very Strong Pull"
        elif pull_score >= 4:
            trend_strength = "🟠 Strong Pull"
        elif pull_score >= 2:
            ...
            trend_strength = "❓ Invalid Score"

        print(f"\n\t+++ Pull Score: {trend_strength} \033[1m[ {pull_score} ]\033[0m")
        print('\t+++ Pull Trend: ', f"{result.get('pull_trend')}")
        print(f'\n\t+++ Delta Shift: \033[1;91m{result.get('delta_shift')}\033[0m') if result.get('delta_shift') <= -20 else \
        print(f'\n\t+++ Delta Shift: {result.get('delta_shift')}')
        print(f"\n\t+++ Bear Power (Current): \033[1m{result.get('delta_10m_1h')}\033[0m")
        print(f"\t+++ Bear Power (Previous): \033[1m{result.get('prev_delta_10m_1h')}\033[0m")
        print(f"\t+++ Bear Power (Analysis): \033[1;91m{str(result.get('delta_10m_1h') < result.get('prev_delta_10m_1h')).upper()}\033[0m")
