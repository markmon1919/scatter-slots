    ...
    while not stop_event.is_set():
        ...
        if current_sec % 10 == 9:
            ...
            if chosen_spin == "normal_spin":
                if random.random() < 0.1 and any(g.get(val, 0) > 0 for val in [state.m10, state.h1, state.h3, state.h6]): # 10% chance to execute spin
                    spin(*random.choice([(True, False), (False, True)]))
        else: # THIS IS TEST
            if current_sec % 2 == 0 and any(g.get(val, 0) > 0 for val in [state.m10, state.h1, state.h3, state.h6]):
                chosen_spin = spin(False, False)
