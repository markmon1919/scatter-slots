...
import argparse, asyncio, hashlib, json, os, platform, random, re, requests, httpx, shutil, subprocess, sys, time, threading#, csv
# import pandas as pd
...
def get_game_data_from_local_api(game: str):
    ...
    response = requests.get("http://localhost:5555/game", params={"name": game})
...
def monitor_game_info(game: str, provider: str, url: str, data_queue: ThQueue):
    ...
    while True:
        try:
            ...
            if data:
                # print("\nDATA prev hash >> ", data.get("min10"), previous_hash)
                current_hash = hashlib.md5(json.dumps(data, sort_keys=True).encode()).hexdigest()
                # print("\nDATA current hash >> ", data.get("min10"), current_hash)
                if current_hash != previous_hash:
                    # print("\nNEW DATA >> ", data.get("min10"), current_hash)
...
if __name__ == "__main__":
    ...
    requests.post("http://localhost:5555/register", json={"name": game})
